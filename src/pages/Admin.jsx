...
import { getCoverUrl, uploadToCloudinary } from '@/lib/cloudinary'
...
export default function Admin() {
  ...
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 20 * 1024 * 1024) {
        showToastMsg("L'image est trop volumineuse (Maximum 20 Mo).", "error")
        return;
      }
      setCoverFile(file)
  ...
  // Upload directement sur Cloudinary - Supabase Storage n'est plus utilisé
  const handleCoverUpload = async () => {
    if (!coverFile) return bookData.cover_url;
    try {
      const cloudinaryUrl = await uploadToCloudinary(coverFile)
      return cloudinaryUrl
    } catch (err) {
      showToastMsg("Erreur lors de l'envoi de l'image vers Cloudinary.", "error")
              ...
              allBooks.filter(b => {
                ...
                <Card key={book.id} className="shadow-sm border border-slate-200 hover:shadow-md transition-all flex flex-col overflow-hidden bg-white">
                  <div className="h-40 w-full bg-slate-50 flex items-center justify-center relative border-b border-slate-100">
                    {book.cover_url ? (
                      <img src={getCoverUrl(book.cover_url, 300)} alt={book.title} loading="lazy" className="object-cover w-full h-full opacity-95" />
                    ) : (
