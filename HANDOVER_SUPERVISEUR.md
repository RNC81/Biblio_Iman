...
- **Table `books` :** L'inventaire central de la bibliothèque. 
  ...
  - Champs vitaux : `title`, `author`, `publisher` (Maison d'édition), `established_by` (Établi par), `translator` (Traducteur), `original_title` (Titre original, texte), `isbn`, `language`, `published_date`, `online_url`, `synopsis`, `private_note`.
